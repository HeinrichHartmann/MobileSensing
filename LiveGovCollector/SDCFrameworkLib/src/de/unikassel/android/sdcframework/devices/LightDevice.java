 ...
 * 
 */
public class LightDevice extends SamplingCausingAndroidSensorDevice
{
  /**
   * The actual sample of the device, updated whenever the handler for sensor
   * changes is called
   */
  private final LightSampleData currentSampleData;
   ...
   *          the context
   */
  public LightDevice( Context context )
  {
    super( SensorDeviceIdentifier.Light, Sensor.TYPE_LIGHT, context,
        SensorManager.SENSOR_DELAY_NORMAL );
    
    this.currentSampleData = new LightSampleData();
    currentSampleData.setLightLevel( -1F );
  }
  ...
  protected boolean update( SensorEvent event )
  {
    ...
    {
      currentSampleData.setLightLevel( event.values[ 0 ] );
      return true;
    }
    return false;
  }
  ...
  {
    return currentSampleData.getLightLevel() >= 0;
  }
