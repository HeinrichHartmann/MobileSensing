...
import de.unikassel.android.sdcframework.devices.PressureDevice;
import de.unikassel.android.sdcframework.devices.PressureDeviceScanner;
...
/**
 * Tests for the pressure device and scanner classes
 * 
 * @author Katy Hilgenberg
 * 
 */
// TODO: find hardware to test
public class TestPressureDeviceAndScanner extends AndroidTestCase
{
  /**
  ...
  protected void setUp() throws Exception
  {
    ...
    isDeviceInGeneralAvailable =
        sensorManager.getSensorList( Sensor.TYPE_PRESSURE ).size() > 0;
    super.setUp();
  }
  ...
  {
    assertTrue( "Test environment does not support pressure",
        isDeviceInGeneralAvailable );
  }
   ...
   */
  public void testPressureDeviceAndScannerCreation()
  {
    testPreconditions();
    
    // create device and scanner and link with each other
    PressureDevice device = new PressureDevice( getContext() );
    PressureDeviceScanner scanner = new PressureDeviceScanner();
    device.setScanner( scanner, getContext() );
    
    assertFalse( "Expected device scanning disabled initally",
        device.isDeviceScanningEnabled() );
    assertEquals( "Wrong device type", SensorDeviceIdentifier.Pressure,
        device.getDeviceIdentifier() );
    assertEquals( "Wrong sensor type", Sensor.TYPE_PRESSURE,
        device.getSensor().getType() );
    assertNotNull( "Expected sample not null", device.getSample() );
    SampleData data = device.getSample().getData();
    assertTrue( "Expected a sample of type PressureSample",
        data instanceof PressureSampleData );
    PressureSampleData convData = (PressureSampleData) data;
    assertTrue( "Wrong initialisation", convData.getPressure() < 0 );
  ...
  public void testSampleUpdate()
  {
    ...
    try
    {
      ...
      PressureSampleData sampleData =
          (PressureSampleData) device.getSample().getData();
      
      assertEquals( "Unexpected sample value for pressure", event.values[ 0 ],
          sampleData.getPressure() );
  ...
  public void testSampling()
  {
    ...
    final PressureDevice device = new PressureDevice( getContext() );
    ...
    for ( Sample sample : observer.observedEvents )
    {
      assertEquals( "Unexpected device identifier",
          SensorDeviceIdentifier.Pressure.toString(), sample.getDeviceIdentifier() );
      assertTrue( "Unexpected sample data type",
          sample.getData() instanceof PressureSampleData );
