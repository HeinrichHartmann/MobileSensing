...
import de.unikassel.android.sdcframework.devices.NetworkLocationDevice;
import de.unikassel.android.sdcframework.devices.NetworkLocationDeviceScanner;
...
/**
 * Tests for the network location device and scanner classes
 * 
 * @author Katy Hilgenberg
 * 
 */
public class TestNetworkLocationDeviceAndScanner extends AndroidTestCase
{
  /*
   * (non-Javadoc)
   * 
  ...
  public void testPreconditions()
  {
    ...
    assertTrue( "The test environment does not support network location",
        locationManager.getProvider( LocationManager.NETWORK_PROVIDER ) != null );
    assertTrue( " network location provider is not enabled in test environment",
   ...
   */
  public void testNetworkLocationDeviceAndScannerCreation()
  {
    testPreconditions();
    
    // create device and scanner and link with each other
    NetworkLocationDevice device = new NetworkLocationDevice( getContext() );
    NetworkLocationDeviceScanner scanner = new NetworkLocationDeviceScanner();
    ...
    assertEquals( "Wrong device type", SensorDeviceIdentifier.NetworkLocation,
    ...
    assertTrue( "Expected a sample of type AccelerometerSample",
        device.getSample().getData() instanceof NetworkLocationSampleData );
  ...
  public void testSampleUpdate()
  {
    ...
    NetworkLocationSampleData sample = (NetworkLocationSampleData) device.getSample().getData();
  ...
  public void testSampling()
  {
    ...
    final NetworkLocationDevice device = new NetworkLocationDevice( getContext() );
    ...
    for ( Sample sample : observer.observedEvents )
    {
      assertEquals( "Unexpected device identifier",
          SensorDeviceIdentifier.NetworkLocation.toString(), sample.getDeviceIdentifier() );
      assertTrue( "Unexpected sample data type",
          sample.getData() instanceof NetworkLocationSampleData );
