...
/**
 * Tests for the light sample data.
 * 
 * @author Katy Hilgenberg
 * 
 */
public class TestLightSampleData extends TestCase
{
  
  /**
   * Test method for construction, setter and getter.
   */
  public final void testLightSampleData()
  {
    LightSampleData sampleData = new LightSampleData();
    
    assertEquals( "Expected light leveluninitialized ", 0.0,
        sampleData.getLightLevel(), 0.000000001 );
    assertNull( "Expected no related data", sampleData.getRelatedData() );
    
    // test setter with getter
    
    float lightLevel = 0.2F;
    sampleData.setLightLevel( lightLevel );
    assertEquals( "Expected light level set", lightLevel,
  ...
  {
    LightSampleData sampleData = createInitializedSampleData();
    ...
    assertEquals( "Expected object equal to a copy construced instance",
        sampleData, new LightSampleData( sampleData ) );
    
    assertFalse( "Expected object not equal to an uninitialized sample",
        sampleData.equals( new LightSampleData() ) );
  }
  ...
  public final void testSerialization()
  {
    ...
    LightSampleData orgSampleData = createInitializedSampleData();
    ...
    try
    {
      ...
      // serialize to object
      sampleData =
          GlobalSerializer.fromXML( LightSampleData.class, sResult );
    }
    catch ( Exception e )
    {
      e.printStackTrace();
   ...
   */
  public static LightSampleData createInitializedSampleData()
  {
    LightSampleData sample = new LightSampleData();
    sample.setLightLevel( 1.2F );
    return sample;
  }
}
