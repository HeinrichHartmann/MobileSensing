...
/**
 * Tests for the pressure sensor sample data.
 * 
 * @author Katy Hilgenberg
 * 
 */
public class TestPressureSampleData extends TestCase
{
  
  /**
   * Test method for construction, setter and getter.
   */
  public final void testPressureSampleData()
  {
    PressureSampleData sampleData = new PressureSampleData();
    

    assertEquals( "Expected distance uninitialized ", 0.0,
        sampleData.getPressure(), 0.000000001 );
    assertNull( "Expected no related data", sampleData.getRelatedData() );
    
    // test setter with getter
    float pressure = 0.2F;
    sampleData.setPressure( pressure );
    assertEquals( "Expected distance set", pressure, sampleData.getPressure() );
    
    // test copy construction
    assertEquals( "Expected equal sample", sampleData,
        new PressureSampleData( sampleData ) );
  }
  ...
  {
    PressureSampleData sampleData = createInitializedSampleData();
    ...
    assertEquals( "Expected object equal to a copy construced instance",
        sampleData, new PressureSampleData( sampleData ) );
    
    assertFalse( "Expected object not equal to an uninitialized sample",
        sampleData.equals( new PressureSampleData() ) );
  }
  ...
  public final void testSerialization()
  {
    ...
    PressureSampleData orgSampleData = createInitializedSampleData();
    ...
    try
    {
      ...
      // serialize to object
      sampleData =
          GlobalSerializer.fromXML( PressureSampleData.class, sResult );
    }
    catch ( Exception e )
    {
      e.printStackTrace();
   ...
   */
  public static PressureSampleData createInitializedSampleData()
  {
    PressureSampleData sample = new PressureSampleData();
    sample.setPressure( 1.2F );
    return sample;
  }
}
