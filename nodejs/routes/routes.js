...
var register = function (server) {
   ...
   */
  server.get('/:uuid/:table', function (req, res, next) {
    if(!checkTable(req.params.table)) {
      res.send(404, 'No resource selected.');
      next();
    }

    var query = 'SELECT * FROM `' + req.params.table + '` WHERE `uuid` = ?'
