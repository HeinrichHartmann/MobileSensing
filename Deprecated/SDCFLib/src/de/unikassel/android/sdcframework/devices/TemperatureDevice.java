 ...
 * 
 */
public class TemperatureDevice extends SamplingCausingAndroidSensorDevice
{
  /**
   * The actual sample of the device, updated whenever the handler for sensor
   * changes is called
   */
  private final TemperatureSampleData currentSampleData;
   ...
   *          the context
   */
  public TemperatureDevice( Context context )
  {
    super( SensorDeviceIdentifier.Temperature, Sensor.TYPE_TEMPERATURE, context,
        SensorManager.SENSOR_DELAY_NORMAL );
    
    this.currentSampleData = new TemperatureSampleData();
    currentSampleData.setTemperature( Float.MIN_VALUE );
  }
  ...
  {
    return currentSampleData.getTemperature() > Float.MIN_VALUE;
  }
   ...
   */
  @Override
  protected synchronized boolean update( SensorEvent event )
  {
    if ( event.values != null && event.values.length == 3 )
    {
      currentSampleData.setTemperature( event.values[ 0 ] );
      return true;
    }
    return false;
  }
