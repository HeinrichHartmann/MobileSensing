...
import de.unikassel.android.sdcframework.transmission.facade.TransferEvent;
    ...
    implements ServiceManager, EventObserver< BatteryLowEvent >
{
  ...
  /**
   * Flag to mirror the expected transfer enabled state, for the duration of a
   * manual triggered sample transfer.
   */
  ...
  public ServiceManagerImpl( Class< ? > controlActivityClass )
  {
    ...
    this.transferStateNeedsReset = new AtomicBoolean();
    this.fileTransferObserver = new EventObserver< TransferEvent >()
    {
      
      @Override
      public void onEvent(
          ObservableEventSource< ? extends TransferEvent > eventSource,
          TransferEvent observedEvent )
      {
        onSuccessfuleFileTransfer( observedEvent );
      }
    };
    setCreated( false );
   ...
   * 
   * @return the fileTransferObserver
   */
  ...
  private UUID getUUID( Context applicationContext )
  {
    ...
    if ( sUuid.length() == 0 )
    {
      ...
      getPreferenceManager().updateUUIDConfiguration( applicationContext, uuid.toString() );
    }
    else
      ...
      ServiceConfiguration updateConfig, boolean force )
  {
    ...
    if ( force
        || ( isBroadcastingSamples != serviceConfig.isBroadcastingSamples() ) )
    {
    ...
    updateBroadcastFrequency( sampleBroadcastService,
        updateConfig.getBroadcastFrequency() );
  }
   ...
   *          the new frequency
   */
  public void updateBroadcastFrequency(
      SampleBroadcastService sampleBroadcastService, long frequency )
  {
    if ( frequency != serviceConfig.getBroadcastFrequency() )
    {
      serviceConfig.setBroadcastFrequency( Long.valueOf( frequency ) );
      ...
      boolean force )
  {
    ...
    if ( force
        || isTransmittingSamples != serviceConfig.isTransmittingSamples() )
    {
      // first update running state
      ...
      transferStateNeedsReset.set( false );
    }
  ...
  {
    if ( !serviceConfig.isTransmittingSamples() )
    {
      getTransferManager().registerEventObserver( getFileTransferObserver() );
      doEnableSampleTransfer( true );
      transferStateNeedsReset.compareAndSet( false, true );
    }
    if ( !getTransferManager().forcedActivation() )
      onSuccessfuleFileTransfer( new TransferEvent() );
  }
   ...
   * 
   * @param observedEvent
   *          the observed transfer event
   */
  protected void onSuccessfuleFileTransfer( TransferEvent observedEvent )
  {
    getTransferManager().unregisterEventObserver( getFileTransferObserver() );
    
    // reset enabled state and unregister ourself after a manual triggered file
    // transfer
    if ( transferStateNeedsReset.compareAndSet( true, false ) )
    {
      doEnableSampleTransfer( false );
    }
  }
}
